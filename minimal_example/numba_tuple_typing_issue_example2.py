...
FunctionInputPackedArgsType = nb.types.Tuple(
    (Array2D, UniTupleOfArrays4x2D, Array2D, Array3D, Array3D, Array2D,
     Array2D))
...
def run_test_case(test_mode):
    ...
    @nb.njit(nb.void(FlattenedParamsType, nb.float64[:]),
             parallel=True)  # 增加 output_results 参数
    def _cpu_parallel_calc_jit_inner_fixed(res_inner_complex_tuple,
        ...
        _data_args_child, indicator_params_child, indicator_params2_child, \
        indicator_result_child, indicator_result2_child, \
        signal_args_child, backtest_args_child = None, None, None, None, None, None, None
        ...
        for idx in nb.prange(conf_count):
            ...
            if test_mode == "JIT-Internal Direct Tuple Pass":
                args_for_unpack_idx = (_data_args_unpacked,
                ...
                # 注意：这里我们**故意保留**这个可能失败的模式来展示问题
                temp_da, temp_ip, temp_ip2, temp_ir, temp_ir2, temp_sa, temp_ba = res_inner_complex_tuple
                args_for_unpack_idx = (temp_da, temp_ip, temp_ip2, temp_ir,
                                       temp_ir2, temp_sa, temp_ba)
            ...
            _params_child = (_data_args_child, indicator_args_child,
                             signal_args_child, backtest_args_child)

            # 调用 _parallel_calc，并传入当前循环迭代的输出位置
            _parallel_calc(_params_child,
                           output_results_inner[idx:idx +
                                                1])  # 传入一个切片，使其成为1D数组
    ...
    try:
        ...
        # 验证计算结果
        expected_value = 1.0 + 1.0 + 1.0  # 因为所有数组都用np.ones初始化，第一个元素都是1.0
        # 确保所有结果都符合预期
        if np.allclose(output_results, expected_value):
            ...
            print(
                f"Calculation for '{test_mode}' FAILED: Results are not as expected."
            )
