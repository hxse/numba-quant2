    ...
    for i in prange(rows):
        for j in prange(cols):
            ...
            for iter_main in range(total_iterations_per_element):
                ...
                intermediate_val_b = (val / 0.11) - (res * (iter_main + 1) * 0.0005)
                ...
                for k in range(3):  # 减少内层循环次数
                    ...
                    if res > 1.5:
                        ...
                        res = -((abs_res_power) ** 0.123) - ((abs_val_power) ** 0.5) * (
                            (abs_res_power) ** 0.05
                        )
                    else:
                    ...
                    if abs_res_check < 1e-15:
                        res = 1e-15 * (1.0 if res > 0 else (-1.0 if res < 0 else 1.0))
                    elif abs_res_check > 1e18:
                        res = 1e18 * (1.0 if res > 0 else -1.0)

            output[i, j] = (
                (res + accumulated_sum / total_iterations_per_element)
                * temp_product
                * (val + 0.005)
                / (res - val + 1e-12)
            )
    ...
    if idx < rows and idy < cols:
        ...
        for iter_main in range(total_iterations_per_element):
            ...
            if intermediate_val_a > 0.6:
                ...
                abs_val = val if val >= 0 else -val
                res = res + intermediate_val_a * intermediate_val_b / (abs_val + 1e-12)
                abs_res_for_sum = res if res >= 0 else -res
                accumulated_sum += abs_res_for_sum * 0.001
        ...
        if (
            arr_out[idx, idy] != arr_out[idx, idy]
            or arr_out[idx, idy] == float("inf")
            or arr_out[idx, idy] == float("-inf")
        ):
            arr_out[idx, idy] = 0.0
...
def run_tests():
    ...
    blockspergrid_x_dummy = (
        dummy_data.shape[0] + threadsperblock[0] - 1
    ) // threadsperblock[0]
    blockspergrid_y_dummy = (
        dummy_data.shape[1] + threadsperblock[1] - 1
    ) // threadsperblock[1]
    blockspergrid_dummy = (blockspergrid_x_dummy, blockspergrid_y_dummy)

    blockspergrid_x_large = (data_size[0] + threadsperblock[0] - 1) // threadsperblock[
        0
    ]
    blockspergrid_y_large = (data_size[1] + threadsperblock[1] - 1) // threadsperblock[
        1
    ]
    ...
    for run_idx in range(1, 3):
        ...
        print("首次调用 (冷启动/编译时间):")
        _, cold_start_time_cpu = calculate_time_wrapper(complex_cpu_calc)(dummy_data)
        ...
        print("\n#### CPU Numba (njit + prange) ####")
        _, calc_time_cpu = calculate_time_wrapper(complex_cpu_calc)(large_data_cpu)
        print(
        ...
        if cuda.is_available():
            ...
            complex_gpu_kernel[blockspergrid_large, threadsperblock](
                d_large_in, d_large_out
            )
            end_gpu_event.record()
            end_gpu_event.synchronize()
            pure_kernel_time_gpu = (
                cuda.event_elapsed_time(start_gpu_event, end_gpu_event) / 1000.0
            )
            print(
