...
@nb.njit(FlattenedParamsType(nb.typeof(None)),
         cache=True)  # None只是占位，实际应该接收params的类型
def _unpack_flatten_simulated(params):
    ...
    arr2_ut = (np.ones(
        (100, 5), dtype=np.float64), np.ones(
            (100, 6), dtype=np.float64), np.ones(
                (100, 7), dtype=np.float64), np.ones((100, 8),
                                                     dtype=np.float64))
...
@nb.njit(
    ...
    cache=True)
def _unpack_flatten_idx_simulated(data_args, indicator_params,
                                  indicator_params2, indicator_result,
                                  indicator_result2, signal_args,
                                  backtest_args, idx):
    ...
    return (_data_args, _indicator_params_child, _indicator_params2_child,
            _indicator_result_child, _indicator_result2_child,
            _signal_args_child, _backtest_args_child)
...
def _parallel_calc_simulated(params_child):
    # 实际计算逻辑，这里简化为打印
    _data_args, indicator_args_child, _signal_args_child, _backtest_args_child = params_child
...
def run_test_case(test_mode):
    ...
    # 模拟 Numba 编译时获取这些函数
    _get_conf_count = nb.njit(nb.int64(nb.typeof(None)),
                              cache=True)(lambda p: 100)  # 模拟返回一个固定数量
    ...
    res = _unpack_flatten(
        params)  # res 是一个包含 7 个元素的元组，其中 res[1] 是 UniTupleOfArrays4x2D
    (data_args, indicator_params, indicator_params2, indicator_result,
     indicator_result2, signal_args, backtest_args) = res
    ...
    def _cpu_parallel_calc_jit_inner(res_inner):
        ...
        _data_args_inner, _indicator_params_inner, _indicator_params2_inner, \
        _indicator_result_inner, _indicator_result2_inner, _signal_args_inner, \
        _backtest_args_inner = res_inner
        ...
        for idx in nb.prange(conf_count):
            ...
            if test_mode == "Explicit Variables (Works)":
                ...
                (_data_args, indicator_params_child, indicator_params2_child,
                 indicator_result_child, indicator_result2_child,
                 signal_args_child, backtest_args_child) = \
                    _unpack_flatten_idx(_data_args_inner, _indicator_params_inner, _indicator_params2_inner,
                                        _indicator_result_inner, _indicator_result2_inner, _signal_args_inner,
                                        _backtest_args_inner, idx)
                ...
                (_data_args, indicator_params_child, indicator_params2_child,
                 indicator_result_child, indicator_result2_child,
                 signal_args_child, backtest_args_child) = \
                    _unpack_flatten_idx(res_inner[0], res_inner[1], res_inner[2], res_inner[3],
                                        res_inner[4], res_inner[5], res_inner[6], idx)
            ...
            # 后续的参数重组和调用保持不变
            indicator_args_child = (indicator_params_child,
                                    indicator_params2_child,
                                    indicator_result_child,
                                    indicator_result2_child)
            _params_child = (_data_args, indicator_args_child,
                             signal_args_child, backtest_args_child)
    ...
    try:
        ...
        print(
            f"Test Case '{test_mode}' FAILED with error: {type(e).__name__}: {e}"
        )
