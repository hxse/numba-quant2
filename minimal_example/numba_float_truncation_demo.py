        ...
        if isinstance(sma_period_input, float):
            ...
            if len(close) >= expected_period_int:
                for i in range(len(close) - expected_period_int + 1):
                    expected_sma_result[i + expected_period_int - 1] = np.sum(
                        close[i:i + expected_period_int]) / expected_period_int

            # 对于 NaN 的比较需要特殊处理
            are_equal = np.allclose(sma_result,
                                    expected_sma_result,
                                    equal_nan=True)
            print(f"计算结果是否符合基于截断整数 ({expected_period_int}) 的预期？ {are_equal}")

        print("✅ 成功！Numba 接受了浮点数输入并正确执行。")
        print("这表明 Numba 能够安全地将此浮点数转换为整数（无精度损失，或进行了可接受的截断）。")
