...
if not cuda.is_available():
    ...
    try:
        ...
        def run_move_gpu_with_dict(input_dict):
            ...
            blocks_per_grid = (data_size +
                               (threads_per_block - 1)) // threads_per_block

            move_gpu_kernel_with_dict[blocks_per_grid,
                                      threads_per_block](input_dict)

        run_move_gpu_with_dict(d_gpu_typed_dict)

        # 如果代码能运行到这里，说明我之前的理解有误，但根据Numba文档，这几乎不可能
        print(
            "CUDA kernel with numba.typed.Dict ran successfully (unexpected).")
