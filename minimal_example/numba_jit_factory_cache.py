...
def numba_wrapper(
    ...
    def decorator(func):
        ...
        key_elements = [func.__qualname__, mode, signature, parallel, cache_enabled]
        ...
        if mode == "njit":
            compiled_func = nb.njit(signature, parallel=parallel, cache=cache_enabled)(
                func
            )
        else:
    ...
    for _ in range(iterations):
        for i in range(rows):
            for j in range(cols):
                ...
                if val_current > 0.5:
                    result_matrix[i, j] = (
                        val_current * 0.99 + val_next_col * 0.005 + val_next_row * 0.005
                    ) * 1.01
                else:
                    result_matrix[i, j] = (
                        val_current * 0.98 + val_next_col * 0.01 + val_next_row * 0.01
                    ) * 0.99
...
@time_wrapper
def get_classic_numba_func(cache_enabled: bool = True, use_global_cache: bool = True):
    """
...
if __name__ == "__main__":
    ...
    # 我们的全局缓存会捕获这个编译结果。
    factory_func_1_on = get_classic_numba_func(
        cache_enabled=True, use_global_cache=True
    )
    ...
    # 因此预期耗时会非常短，接近零。
    factory_func_2_on = get_classic_numba_func(
        cache_enabled=True, use_global_cache=True
    )
    ...
    input_matrix_off = np.random.rand(matrix_rows, matrix_cols).astype(np.float64)
    print("数据生成完毕。")
    ...
    factory_func_1_off = get_classic_numba_func(
        cache_enabled=True, use_global_cache=False
    )
    ...
    factory_func_2_off = get_classic_numba_func(
        cache_enabled=True, use_global_cache=False
    )
