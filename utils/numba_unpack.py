...
from utils.data_types import default_types, get_params_signature, get_params_child_signature
...
def initialize_outputs(tohlcv,
    ...
    sma_result = np.full((conf_count, sma_rows, sma_output_dim),
                         np.nan,
                         dtype=np_float_type)

    sma2_rows = tohlcv_rows if indicator_enabled[sma2_spec["id"]] else min_rows
    sma2_result = np.full((conf_count, sma2_rows, sma2_output_dim),
                          np.nan,
                          dtype=np_float_type)

    bbands_rows = tohlcv_rows if indicator_enabled[
        bbands_spec["id"]] else min_rows
    bbands_result = np.full((conf_count, bbands_rows, bbands_output_dim),
                            np.nan,
                            dtype=np_float_type)

    indicator_result = (sma_result, sma2_result, bbands_result)

    sma_rows2 = tohlcv2_rows if indicator_enabled2[
        sma_spec["id"]] else min_rows
    sma_result2 = np.full((conf_count, sma_rows2, sma_output_dim),
                          np.nan,
                          dtype=np_float_type)

    sma2_rows2 = tohlcv2_rows if indicator_enabled2[
        sma2_spec["id"]] else min_rows
    sma2_result2 = np.full((conf_count, sma2_rows2, sma_output_dim),
                           np.nan,
                           dtype=np_float_type)

    bbands_rows2 = tohlcv2_rows if indicator_enabled2[
        bbands_spec["id"]] else min_rows
    bbands_result2 = np.full((conf_count, bbands_rows2, bbands_output_dim),
                             np.nan,
                             dtype=np_float_type)
...
def unpack_params_child(mode, cache=True, dtype_dict=default_types):
    ...
    def _unpack_params_child(params, idx):
        '''
        ...
        indicator_params_child = (sma_params[idx], sma2_params[idx],
                                  bbands_params[idx])
        indicator_params2_child = (sma_params2[idx], sma2_params2[idx],
                                   bbands_params2[idx])
        indicator_result_child = (sma_result[idx], sma2_result[idx],
                                  bbands_result[idx])
        indicator_result2_child = (sma_result2[idx], sma2_result2[idx],
                                   bbands_result2[idx])

        indicator_args_child = (indicator_params_child,
                                indicator_params2_child, indicator_enabled,
...
def get_conf_count(mode, cache=True, dtype_dict=default_types):
    ...
    return numba_wrapper(mode, signature=signature,
                         cache_enabled=cache)(_get_conf_count)
