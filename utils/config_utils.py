...
def get_params(num,
    ...
    indicator_params = get_indicator_params(num,
                                            update_params=indicator_update,
                                            dtype_dict=dtype_dict)
    indicator_enabled = get_indicator_enabled(update_params=indicator_enabled,
                                              dtype_dict=dtype_dict)
    signal_params = get_signal_params(params=signal_params,
                                      dtype_dict=dtype_dict)
    backtest_params = get_backtest_params(num,
                                          params=backtest_params,
                                          dtype_dict=dtype_dict)

    indicator_params2 = get_indicator_params(num,
                                             update_params=indicator_update2,
                                             dtype_dict=dtype_dict)
...
def get_indicator_params(num: int = 1,
                         update_params: dict = {},
                         dtype_dict=default_dtype_dict) -> dict:
    """
    ...
    for k, v in default_template.items():
        assert v["param_count"] == len(v["default_params"]), f"默认参数数量不对 {v["param_count"]} { len(v["default_params"])}"
        assert v["id"] not in id_history, f"指标id不允许重复 {k}"
        id_history.add(v["id"])


    # 初始化默认参数
    default_params = {k: [v["default_params"]for i in range(num)] for k, v in default_template.items()}
