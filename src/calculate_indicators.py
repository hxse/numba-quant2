...
def calc_indicators(mode, cache=True, dtype_dict=default_types):
    ...
    _calculate_sma_wrapper = calculate_sma_wrapper(
        mode, cache=cache, dtype_dict=dtype_dict
    )

    _calculate_bbands_wrapper = calculate_bbands_wrapper(
        mode, cache=cache, dtype_dict=dtype_dict
    )

    def _calc_indicators(params_child):
        (data_args, indicator_args, signal_args, backtest_args, temp_args) = (
            params_child
        )
        ...
        (int_temp_array_child, float_temp_array_child, bool_temp_array_child) = (
            temp_args
        )
